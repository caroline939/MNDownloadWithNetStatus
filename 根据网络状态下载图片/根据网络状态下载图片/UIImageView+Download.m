...
{
    [self MN_setImageWithOriginalImageURL:originalImageURL thumbnailImageURL:thumbnailImageURL placeholderImage:placeholderImage completed:nil];
}
...
- (void)MN_setImageWithOriginalImageURL:(NSString *)originalImageURL thumbnailImageURL:(NSString *)thumbnailImageURL placeholderImage:(UIImage *)placeholderImage completed:(SDWebImageCompletionBlock)completedBlock
{
    ...
    if (originalImage) { // 如果内存\沙盒缓存有原图，那么就直接显示原图（不管现在是什么网络状态）
        ...
        if (mgr.isReachableViaWiFi) { // 在使用Wifi, 下载原图
            ...
            //    [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"notalwaysDownloadOriginalImage"];
...
#warning 从沙盒中读取用户的配置项：在3G\4G环境是否仍然下载原图
            ...
            NSLog(@"notalwaysDownloadOriginalImage---%d",notalwaysDownloadOriginalImage);
            NSLog(@"%d",notalwaysDownloadOriginalImage);
            if (notalwaysDownloadOriginalImage) { // 下载小图
    ...
    NSLog(@"大图-%@",originalImageURL);
    NSLog(@"小图-%@",thumbnailImageURL);
}

- (void)MN_setImageWithOriginalImageURL:(NSString *)originalImageURL thumbnailImageURL:(NSString *)thumbnailImageURL
{
    [self MN_setImageWithOriginalImageURL:originalImageURL thumbnailImageURL:thumbnailImageURL placeholderImage:nil completed:nil];
}
...
{
    [self MN_setImageWithOriginalImageURL:originalImageURL thumbnailImageURL:thumbnailImageURL placeholderImage:nil completed:completedBlock];
}
@end
